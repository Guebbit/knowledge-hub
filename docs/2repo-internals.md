...
### Layer 7b — Architecture layer (optional, `graphify-out/arch/`)
...
- **`arch`** behaves exactly like `wiki` (its own optional doc layer, then index+context refresh, no state write), but produces component/topic pages + Mermaid diagrams in `graphify-out/arch/` instead of per-file pages.
